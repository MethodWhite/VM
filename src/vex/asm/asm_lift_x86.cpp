 ...
 *  Ver asm_lift_x86.h. */
#include "vex/asm/asm_lift_x86.h"

#include "vex/asm/asm_cfg.h"       // build_asm_cfg
#include "vex/asm/asm_effects.h"   // asm_canonical_reg
#include "vex/asm/asm_lift_core.h" // core neutro
#include "vex/asm/asm_phys_reg.h"  // asm_x86_gp_index
...
int reg_info(const std::string &tok, std::string &canon, bool &is_high) {
    ...
    uint16_t w = 0;
    const int idx = vex::asm_x86_gp_index(s, &w);
...
bool lift_x86(
    ...
    const vex::AsmCfg cfg = vex::build_asm_cfg(vex::instr_db::Isa::X86, body);
    ...
    hooks.term_start = [&](const vex::AsmBasicBlock &bb) -> uint32_t {
        ...
        if (bb.term == vex::AsmTerm::CondBranch ||
            bb.term == vex::AsmTerm::UncondJump)
            return bb.last;
        return bb.last + 1u;
    };
    hooks.branch_cond = [&](const vex::AsmBasicBlock &bb) -> ir::IrValueId {
