...
bool analyze_loop(const IrFunction &fn, const analysis::LoopFacts &lf,
    ...
    //    comprobacion estructural: usa la membresia del bucle.  Un CONST en el
    //    header (p.ej. la cota literal de un `while (i < 100000)`) es invariante:
    //    solo se rechazan definiciones en bloques del CUERPO (loop-carried).
    if (out.iv.bound != IR_NO_VALUE && out.iv.bound < def_block.size()) {
        const int db = def_block[out.iv.bound];
        if (db >= 0 && db != (int)out.st.header && out.st.contains((IrBlockId)db))
            return false;
    }
...
bool ir_pass_unroll(IrFunction &fn, int factor) {
    ...
    bool changed = false;
    const char *f_env = std::getenv("VESTA_UNROLL_FACTOR");
    const int f_override = f_env ? atoi(f_env) : 0;
    for (const LoopInfo &li : eligible) {
        int U;
        if (factor > 0 || f_override > 0) { // override manual (testing).
            U = (factor > 0) ? factor : f_override;
        } else {
