...
struct CfgHooks {
    ...
    std::function<ir::IrValueId(const vex::AsmBasicBlock &)> branch_cond;
    ...
    std::function<uint32_t(const vex::AsmBasicBlock &)> term_start;
};
...
inline bool lift_cfg_neutral(LiftCtx &c, const vex::AsmCfg &cfg, const CfgHooks &hooks,
                      uint32_t &out_exit) {
    ...
    for (size_t i = 0; i < nb; ++i) {
        const vex::AsmBasicBlock &bb = cfg.blocks[i];
        c.block = irb[i];
